...
const AppState = {
    ...
    isMobileSidebarOpen: false, // Track mobile sidebar state
    mockData: { 
        students: [
            { 
...
// const themeToggle = document.getElementById('themeToggle'); // This might be for the static one, dynamic one is handled in renderSidebar
...
function applyTheme() {
    ...
    if (themeToggleInSidebar) {
     ...
     // Also update the main theme toggle button if it exists (e.g. static one in sidebar header before dynamic render)
    const mainThemeToggle = document.getElementById('themeToggle');
    if (mainThemeToggle && !themeToggleInSidebar) { // Only if dynamic one isn't present yet
...
function renderSidebar() { 
    ...
    if (user && sidebarHeader) { 
        ...
        // Add listener for dynamically created close button
        const dynamicCloseButton = document.getElementById('closeSidebarButtonDynamic');
        if (dynamicCloseButton) {
            dynamicCloseButton.addEventListener('click', () => toggleMobileSidebar(true));
        }
    }
    ...
    const createCategory = (id, icon, text, sublinks) => `
        <div class="sidebar-category">
            <div class="flex items-center space-x-3 cursor-pointer" onclick="toggleSubmenu('${id}')">
                ...
                <div class="card p-6 md:p-8 rounded-lg">
                    ...
                    <div class="mt-8">
                        ...
                        <ul id="recentMintsLog" class="list-disc list-inside text-sm text-slate-600 space-y-1.5 max-h-48 overflow-y-auto p-3 bg-slate-50 rounded-md border">
                            ${AppState.mockData.school.cardanoWallet.recentMints.length > 0 ? AppState.mockData.school.cardanoWallet.recentMints.map(m => `<li>${m.type} for ${m.studentName} - ${m.date} (Tx: ${m.txId.substring(0,10)}...)</li>`).join('') : '<li>No recent school-initiated mints.</li>'}
                        </ul>
                    </div>
                     <div class="mt-8">
            ...
            <div class="card p-6">
                ...
                <div class="max-h-80 overflow-y-auto space-y-3">
                    ${[...student.academicTimeline, ...student.readingLogs, ...student.participation]
                        .sort((a,b) => new Date(b.date || b.year || 0) - new Date(a.date || a.year || 0)) // Ensure valid dates for sorting
...
function updateMobileNavActiveState() {
    ...
    if (targetPage.includes('Dashboard') || targetPage.includes('Wallet') || targetPage.includes('Passport')) {
        targetPage = 'dashboard'; // Group various dashboard/profile views under 'dashboard' for mobile nav
    } else if (targetPage === 'messaging') {
        targetPage = 'messages';
    } else if (targetPage === 'events') {
        targetPage = 'events';
    } else if (targetPage === 'studentDashboard' || targetPage === 'teacherDashboard' || targetPage === 'adminDashboard' || targetPage === 'parentDashboard') {
        // A more generic "profile" or role-specific main page could be here
        // For now, let's assume 'profile' maps to the main dashboard for the user
        targetPage = 'profile'; 
    }
...
function navigateTo(page){
    ...
    if(sidebarNav) { // Check if sidebarNav exists
        sidebarNav.querySelectorAll('.sidebar-link.active').forEach(l=>l.classList.remove('active'));
    }
    renderSidebar();
    renderMainContent();
    window.scrollTo({top:0,behavior:'smooth'});
    if (AppState.isMobileSidebarOpen) { // Close mobile sidebar on navigation
...
function showLoginPage(){
    ...
    if (AppState.isMobileSidebarOpen) { // Ensure sidebar is closed on logout
        toggleMobileSidebar(true);
    }
}
...
document.addEventListener('DOMContentLoaded', () => { 
    ...
    // Note: The closeSidebarButton in the HTML is static. 
    // If renderSidebar replaces it, the listener needs to be attached to the dynamic one.
    // For the static one (if it remains for some reason):
    const staticCloseSidebarButton = document.getElementById('closeSidebarButton');
    if (staticCloseSidebarButton) {
        staticCloseSidebarButton.addEventListener('click', () => toggleMobileSidebar(true));
    }
    // Listener for dynamically created close button is added in renderSidebar
    ...
    if (mobileNav) {
        ...
        mobileNavItems.forEach(item => {
            item.addEventListener('click', (e) => {
                e.preventDefault();
                const page = item.dataset.pageMobile;
                let targetPage = page;

                // Map mobile nav pages to actual AppState pages
                if (page === 'dashboard' || page === 'profile') { // Consolidate profile/dashboard for mobile
                    switch(AppState.currentUser?.role) {
                        ...
                        default: targetPage = AppState.currentPage; // fallback
                    }
                ...
                // toggleMobileSidebar(true); // navigateTo already closes it
                // updateMobileNavActiveState(); // navigateTo already updates it
            });
        });
    }
